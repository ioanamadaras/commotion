...
export default function BoardModal({ boardId }: { boardId: string }) {
	...
	useEffect(() => {
		...
		async function loadBoard() {
			try {
				...
				seteditorUserIds(data.editorUserIds ?? []);
				...
				if (userIds.length > 0) {
					const users = await api(
						`/user/lookup?ids=${encodeURIComponent(userIds.join(','))}`,
					);
					if (!cancelled) {
						rememberUsers(users);
					}
	...
	useEffect(() => {
		...
		const timeout = window.setTimeout(async () => {
			try {
				const users = await api(
					`/user/search?query=${encodeURIComponent(trimmed)}`,
				);
				if (!cancelled) {
					setResults(users);
					setKnownUsers((prev) => {
	...
	const currentEditors = useMemo(
		() => editorUserIds.map((id) => knownUsers[id]).filter(Boolean),
		[editorUserIds, knownUsers],
	);
	...
	function addUser(user: UserType, role: UserRole) {
		...
		const nextEditors = editorUserIds.filter((id) => id !== user._id);
	...
	function removeUser(userId: string) {
		const nextEditors = editorUserIds.filter((id) => id !== userId);
										...
										<div className="flex max-h-[min(28vh,16rem)] flex-col gap-2 overflow-y-auto pr-1">
											{currentEditors.length === 0 ? (
												...
												currentEditors.map((user) => (
													<div
														key={user._id}
														className="flex items-center justify-between gap-2 rounded-md bg-[var(--bg-darker)] px-2 py-1"
													>
