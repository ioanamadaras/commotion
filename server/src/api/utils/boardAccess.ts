...
export const permissionOf = async (
    ...
    user: { _id?: unknown; role?: string } | null | undefined,
    ...
    if (user?.role === 'guest' && role !== 'viewer') {
        return null;
    }
...
export const setBoardPermissions = async (
    ...
    const normalizedEditors = [...new Set(editorUsersIds.map(String).map((id) => id.trim()).filter(Boolean))]
        .filter((userId) => userId !== ownerValue);
    const normalizedViewers = [...new Set(viewerUserIds.map(String).map((id) => id.trim()).filter(Boolean))]
        .filter((userId) => !normalizedEditors.includes(userId));

    const desiredByUserId = new Map<string, BoardMemberRole>();
    normalizedEditors.forEach((userId) => desiredByUserId.set(userId, 'editor'));
    normalizedViewers.forEach((userId) => desiredByUserId.set(userId, 'viewer'));
    ...
    await Promise.all(
        [...desiredByUserId.entries()].map(([userId, role]) => boardMemberModel.findOneAndUpdate(
            { boardId, userId },
            { $set: { role } },
    ...
    return {
        editorUsersIds: normalizedEditors,
