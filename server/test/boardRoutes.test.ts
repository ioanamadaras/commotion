...
describe("board routes", () => {
    ...
    it("forbids non-owners from changing board permissions", async () => {
        ...
        const response = await request(app)
            ...
            .send({ editorUserIds: [], viewerUserIds: ["user-2"] });

        expect(response.status).toBe(403);
        expect(response.body).toEqual({ error: "Forbidden" });
    });

    it("lets owners change board permissions", async () => {
        mockedGetAuthenticatedUser.mockResolvedValue({
            _id: "user-1",
            userType: "user",
            toObject: () => ({
        ...
        jest.mocked(boardModel).findById.mockResolvedValue(board);
        jest.mocked(boardMemberModel).findOne.mockResolvedValue({
            boardId: "board-1",
            userId: "user-1",
        });

        // apelam endpointul de schimbare de permisiuni
        const response = await request(app).patch("/board/board-1/permissions");

        // verficam validitatea raspunsului
        expect(response.status).toBe(200);
        expect(response.body.owner).toEqual("user-1");
    });
});
